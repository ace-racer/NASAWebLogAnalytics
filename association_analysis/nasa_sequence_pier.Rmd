...
testegs$preds = apply(testegs,1,function(X) makepreds(X["webpage"], rulesDF)) # something wrong here getting preds like rules
```

```{r}
# Testing why it doesn't work... 
#antecedent = paste("<{",'/shuttle/countdown/',"}> =>",sep="") # NOTE: rules here has '<' and '>' surrounding
#firingrules = rulesDF[grep(antecedent, rulesDF$rule,fixed=TRUE),1]
#firingrules
```
```{r}
...
correctpreds = sum(apply(userpreds,1,function(X) checkpreds(X["preds"],X["sequenceID"])))# 0??
